...
3. Now create another text file that contains the number of words in the **first 25 lines** of the file with the name `Pacifici-25wordcount.txt` in your `class-assignments/homework-two/`.
...
The `cut` command allows one to extract data columns that are formatted in a `.csv` format (*csv* stands for "comman separated values", although the delimiting (separating) character between values of the file (a.k.a., *the delimiter*) does not necessary need to be a comma).
...
2. We don;'t want all of the data, just columns 2-6, so use the `cut` option with the properly specific delimiter and column range values.
